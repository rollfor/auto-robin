...
-- Vendored from RollFor's own test harness (RollFor/test/utils.lua in rollfor/rollfor).
--
-- This addon is tested against RollFor in the deps/rollfor submodule, pinned to a release
-- tag. The parts of the harness every addon shares are reached for in deps/rollfor/test/common;
-- this file is copied rather than reached for, so the extension's suite stands on its own --
-- at the cost of drifting from core's copy.
...
-- EXTENSION: RollFor comes from deps/rollfor, and it comes before the addon's own src/ --
-- both ship a src/, and core has to win those lookups. The repo root follows, for this
-- harness' addon-qualified names, then deps/rollfor itself, for test/common.
package.path = "./?.lua;" .. package.path ..
    ";../../deps/rollfor/RollFor/src/?.lua;../../deps/rollfor/RollFor/src/libs/?.lua;../../deps/rollfor/RollFor/src/libs/LibStub/?.lua;../src/?.lua;../../?.lua;../../deps/rollfor/?.lua"
...
function M.load_extension()
  ...
  require( "src/AutoRoundRobinQueuesContentTransformer" )
  require( "src/AutoRoundRobinAddPlayerFrame" )
  require( "src/AutoRoundRobinQueuesTab" )
  require( "src/AutoRoundRobinSimulator" )
  require( "src/RoundRobinWidgets" )
  require( "src/OptionsPage" )
  require( "RollForAutoRobin" )
end

function M.load_roll_for()
  -- EXTENSION: RollFor's main, then this addon on top of it -- the order the client
  -- produces from `## Dependencies: RollFor`. Requiring "RollForAutoRobin" here
